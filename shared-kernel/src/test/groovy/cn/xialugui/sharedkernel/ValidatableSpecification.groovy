package cn.xialugui.sharedkernel
...
/*
    ...
    def ifViolated(Set<ConstraintViolation> constraintViolations, Closure action) {
        if (null != constraintViolations && constraintViolations.size() > 0) {
            action.call()
        }
        return true
    ...
    def ifViolatedOrElse(Set<ConstraintViolation> constraintViolations, Closure action, Closure otherAction) {
        boolean result = (null != constraintViolations && constraintViolations.size() > 0)
        if (result) {
            action.call(constraintViolations[0])
        } else {
            otherAction.call(result)
        }
    }
